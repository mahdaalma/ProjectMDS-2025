...
```{r}
#Universitas
univ <- read.csv("D:/MDS/ProjectMDS-2025/Dataset Kelompok_2/UNIVERSITAS.csv", stringsAsFactors = FALSE)
head(univ)  # Cek apakah data terbaca dengan benar

#Jalur Masuk
jalur_masuk <- read.csv("D:/MDS/ProjectMDS-2025/Dataset Kelompok_2/jalur masuk1.csv", stringsAsFactors = FALSE)
...
for (i in 1:nrow(univ)) {
  query <- sprintf("INSERT INTO Universitas (id_univ, id_wilayah, nama_univ, akred_univ) VALUES ('%s', '%s', '%s', '%s')",           univ$id_univ[i], univ$id_wilayah[i], univ$nama_univ[i], univ$akred_univ[i])
  dbExecute(con_db, query)
}
